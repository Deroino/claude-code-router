...
class ApiClient {
  ...
  async testModel(provider: string, model: string, message?: string): Promise<{ success: boolean; status?: number; response?: string }> {
    return this.post<{ success: boolean; status?: number; response?: string }>("/model-test", {
      provider,
      model,
      message,
    });
